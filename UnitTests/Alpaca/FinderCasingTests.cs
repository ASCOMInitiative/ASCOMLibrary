...
namespace ASCOM.Alpaca.Tests
{
    ...
    public class FinderCasingTests
    {
        ...
        /// <summary>
        /// Polls <see cref="Finder.BroadcastResponses"/> until the requested number of raw responses
        /// has been recorded, or the timeout elapses.
        /// </summary>
        private static async Task<bool> WaitForBroadcastResponseCount(Finder finder, int expectedCount, int timeoutMs = ResponseTimeoutMs)
        {
            DateTime deadline = DateTime.UtcNow.AddMilliseconds(timeoutMs);
            while (DateTime.UtcNow < deadline)
            {
                if (finder.BroadcastResponses.Count >= expectedCount)
                    return true;
                await Task.Delay(25, TestContext.Current.CancellationToken);
            }
            return false;
        ...
        [Fact]
        public async Task RepeatedSearches_ReceiveSubsequentResponses()
        {
            const int alpacaPort = 20100;
            const int discoveryPort = 32267;
            ...
            using (var finder = new Finder())
            {
                ...
                bool firstResponseReceived = await WaitForBroadcastResponse(finder);
                Assert.True(firstResponseReceived, "The first discovery response was not received.");

                // Allow responses generated by the first search to be processed before recording
                // the baseline count for the second search.
                await Task.Delay(ResponseTimeoutMs, TestContext.Current.CancellationToken);
                int firstResponseCount = finder.BroadcastResponses.Count;

                finder.Search(discoveryPort, true, false);

                bool subsequentResponseReceived = await WaitForBroadcastResponseCount(finder, firstResponseCount + 1);
                Assert.True(subsequentResponseReceived, "A response to the second discovery search was not received.");
            }
        }
    }
}
