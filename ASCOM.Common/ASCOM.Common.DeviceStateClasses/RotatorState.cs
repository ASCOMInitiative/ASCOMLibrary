...
namespace ASCOM.Common.DeviceStateClasses
{
    ...
    public class RotatorState
    {
        ...
        public RotatorState(ArrayList deviceStateArrayList, ILogger TL)
        {
                ...
                try
                {
                    ...
                    switch (stateValue.Name)
                    {
                        ...
                        case nameof(IRotatorV4.MechanicalPosition):
                            ...
                            {
                                TL?.LogMessage(LogLevel.Debug, className, $"MechanicalPosition - Ignoring exception: {ex.Message}");
                            }
                            TL?.LogMessage(LogLevel.Debug, className, $"MechanicalPosition has value: {MechanicalPosition.HasValue}, Value: {MechanicalPosition}");
                            break;
