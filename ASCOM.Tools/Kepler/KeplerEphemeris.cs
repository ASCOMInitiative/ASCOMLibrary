...
namespace ASCOM.Tools.Kepler
{
    ...
    public class Ephemeris
    {
        ...
        private KeplerGlobalCode.Orbit m_e = new KeplerGlobalCode.Orbit(0.0); // Elements, etc for minor planets/comets, etc.
        ...
        public BodyType BodyType
        {
            ...
            {
                if (!m_bTypeValid)
                    throw new ValueNotSetException("KEPLER:BodyType BodyType has not been set");
                return m_Type;
            }
            set
            {
        ...
        public double[] GetPositionAndVelocity(double tjd)
        {
            ...
            if (!m_bTypeValid)
                throw new ValueNotSetException("Kepler:GetPositionAndVelocity Body type has not been set");
        ...
        public string Name
        {
            get
            {
                if (string.IsNullOrEmpty(m_Name))
                    throw new ValueNotSetException("KEPLER:Name Name has not been set");
                return m_Name;
            }
            set
            {
        ...
        public Body Number
        {
            ...
            {
                if (!m_bNumberValid)
                    throw new ValueNotSetException("KEPLER:Number Planet number has not been set");
                return m_Number;
            }
            set
            {
