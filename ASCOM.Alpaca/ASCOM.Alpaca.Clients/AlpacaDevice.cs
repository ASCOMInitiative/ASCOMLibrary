...
namespace ASCOM.Alpaca.Clients
{
    ...
    public class AlpacaClient
    {
        ...
        public static T GetDevice<T>(AscomDevice ascomDevice, int establishConnectionTimeout,
                                  int standardDeviceResponseTimeout, int longDeviceResponseTimeout, uint clientNumber, string userName, string password, bool strictCasing, ILogger logger) where T : AlpacaDeviceBaseClass, new()
        {
            // Validate that the ascomDevice parameter is not null
            if (ascomDevice is null)
                throw new InvalidValueException($"AlpacaClient.GetDevice - The supplied ascomDevice parameter is null");
        ...
        {
            return (T)Activator.CreateInstance(typeof(T), new object[] { serviceType, ipAddressString, portNumber, remoteDeviceNumber, true, null });
        }
